        ...
        <div className="relative">
          ...
          >
            <g style={{
              ...
              <path d="M170 95 Q185 75 195 60 Q200 52 192 50 Q184 48 180 58 Q175 72 167 90" fill="#f59e0b" stroke="#d97706" strokeWidth="1.5" />
              <path d="M190 53 Q188 49 192 50" fill="#fef3c7" stroke="none" />
            </g>
            ...
            {!blinking && (<><circle cx="44" cy="66" r="2" fill="white" /><circle cx="64" cy="66" r="2" fill="white" /></>)}
            <ellipse cx="52" cy="78" rx="3.5" ry="2.5" fill="#f97316" />
            <path d={`M52 80 Q46 ${85 + (faceIndex % 2) * 2} 44 83`} fill="none" stroke="#9a3412" strokeWidth="1.2" strokeLinecap="round" />
            <path d={`M52 80 Q58 ${85 + (faceIndex % 2) * 2} 60 83`} fill="none" stroke="#9a3412" strokeWidth="1.2" strokeLinecap="round" />
        ...
        <div className="fixed bottom-24 right-4 z-50 w-80 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden" style={{ maxHeight: '450px' }}>
          ...
          <div className="p-3 h-64 overflow-y-auto bg-orange-50/30">
            {messages.map((msg, i) => (
              <div key={i} className={`flex mb-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[82%] px-3 py-2 rounded-2xl text-sm leading-relaxed ${msg.role === 'user' ? 'bg-orange-400 text-white rounded-br-md' : 'bg-white text-gray-700 rounded-bl-md border border-orange-200'}`}>
                  {msg.text}
                </div>
              </div>
            ))}
          ...
          <div className="p-3 border-t border-gray-100 bg-white">
            <div className="flex space-x-2">
              <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleChat()} placeholder="和果果仁说点什么..." className="flex-1 px-3 py-2 border border-gray-200 rounded-xl focus:outline-none focus:border-orange-400 text-sm" />
              <button onClick={handleChat} disabled={!input.trim() || isTyping} className="px-4 py-2 bg-orange-400 text-white rounded-xl hover:bg-orange-500 transition disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium">发送</button>
            </div>
          </div>
        </div>
      )}
