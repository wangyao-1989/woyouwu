  ...
  return new Promise((resolve, reject) => {
    ...
    protocol.get(url, (response) => {
      if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
        const rproto = response.headers.location.startsWith('https') ? https : http;
        rproto.get(response.headers.location, (res2) => {
          const chunks = [];
...
async function main() {
  ...
  const models = {
    Item: require('../models/Item'), Project: require('../models/Project'),
    Article: require('../models/Article'), Inspiration: require('../models/Inspiration'),
  ...
  for (const [modelName, config] of Object.entries(collections)) {
    ...
    for (const doc of all) {
      ...
      try {
        ...
        if (config.imgField === 'images') { doc.images = [imgPath]; }
        else { doc[config.imgField] = imgPath; }
